...
export function getSolution(codeKey: string): Solution | null {
  return (
    ...
    ARVORES_SOLUTIONS[codeKey] ??
    null
  )
}

// ── Curso Árvores Binárias ─────────────────────────────────────
export const ARVORES_SOLUTIONS: Record<string, Solution> = {
  'arvores-ex1': {
    explanation:
      'Busca descarta metade por passo: 40 existe (True); 99 não foi inserido (False).',
    code: `class No:
...
print(f"busca 99: {bst.buscar(99)}")`,
  ...
  'arvores-ex2': {
    explanation:
      'Mínimo = sempre à esquerda até o fim (20). Máximo = sempre à direita (80).',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(f"max: {maximo(raiz)}")`,
  ...
  'arvores-projeto': {
    explanation:
      'Cada nó: min < valor < max (intervalo herdado dos ancestrais). Inserções respeitam → True.',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(f"valida: {validar(raiz)}")`,
  ...
  'arvores-p-ex1': {
    explanation:
      'em-ordem (esq → raiz → dir) no BST sai ORDENADO: [20, 30, 40, 50, 60, 70, 80].',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(em_ordem(raiz))`,
  ...
  'arvores-p-ex2': {
    explanation:
      'pré-ordem (raiz → esq → dir): 50, subárvore esquerda (30, 20, 40), direita (70).',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(pre_ordem(raiz))`,
  ...
  'arvores-p-projeto': {
    explanation:
      'pós-ordem (esq → dir → raiz): folhas primeiro, raiz por último. 20, 40, 30, 70, 50.',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(pos_ordem(raiz))`,
  ...
  'arvores-o-ex1': {
    explanation:
      'altura = 1 + max(esq, dir). 7 nós balanceados → altura 3 (log₂(8) ≈ 3).',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(f"altura: {altura(raiz)}")`,
  ...
  'arvores-o-ex2': {
    explanation:
      'contar = 1 + contar(esq) + contar(dir). 50, 30, 70, 20, 40, 60, 80 = 7 nós.',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
print(f"nos: {contar(raiz)}")`,
  ...
  'arvores-o-projeto': {
    explanation:
      'Espelhar troca esq/dir em cada nó. Em-ordem do espelho sai invertido: 70, 50, 40, 30, 20.',
    code: `class No:
    def __init__(self, valor):
        self.valor = valor
        self.esq = None
...
// ── Curso Grafos ──────────────────────────────────────────────
export const GRAFOS_SOLUTIONS: Record<string, Solution> = {
  'grafos-ex1': {
    explanation:
      'Fila FIFO: A → B, C → vizinhos de B (D). Ordem por níveis: A, B, C, D.',
...
print(bfs(grafo, "A"))`,
  ...
  'grafos-ex2': {
    explanation:
      'BFS garante o mínimo: A→C (1) → D (2) → E (3). A fila guarda (nó, distância).',
    code: `from collections import deque
...
print(f"A ate E: {distancia(grafo, 'A', 'E')}")`,
  ...
  'grafos-projeto': {
    explanation:
      'BFS com níveis: ana (0) → bob/carol (1) → duda (2). Nível 2 = ["duda"].',
    code: `from collections import deque
...
print(f"nivel 2: {recomendacoes(grafo, 'ana', 2)}")`,
  ...
  'grafos-d-ex1': {
    explanation:
      'DFS vai fundo: A → primeiro vizinho B → D → volta → C. Profundidade primeiro!',
    code: `def dfs(grafo, no, visitados=None):
...
print(dfs(grafo, "A"))`,
  ...
  'grafos-d-ex2': {
    explanation:
      'F é isolado — DFS de F não alcança nada (False). A→E existe via B→D (True).',
    code: `def existe_caminho(grafo, inicio, alvo, visitados=None):
    if visitados is None:
        visitados = set()
    if inicio == alvo:
        return True
...
print(f"F->A: {existe_caminho(grafo, 'F', 'A')}")`,
  ...
  'grafos-d-projeto': {
    explanation:
      'Cada nó não visitado no loop externo = novo componente. A-B-C, D-E, F → 3.',
    code: `def componentes(grafo):
    visitados = set()
    total = 0
    for no in grafo:
...
print(f"componentes: {componentes(grafo)}")`,
  ...
  'grafos-a-ex1': {
    explanation:
      'Aresta u→v = "u antes de v". python (grau 0) → pip → pandas → app.',
    code: `from collections import deque
...
print(ordem_topologica(grafo))`,
  ...
  'grafos-a-ex2': {
    explanation:
      'Heap sempre expande o menor. A→B→C→D = 1+2+1 = 4 (melhor que A→C→D = 5).',
    code: `import heapq
...
print(f"A->D: {d['D']}")`,
  ...
  'grafos-a-projeto': {
    explanation:
      'SP→MG→ES = 500+300 = 800. SP→RJ→ES = 400+500 = 900 é pior. Dijkstra escolhe o menor.',
    code: `import heapq
